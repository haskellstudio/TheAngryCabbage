  ...
  (lambda (expr env-cps k^)
    (match expr
      ...
      [`(let/cc ,body) (value-of-cps body (lambda (y k^^) (if (zero? y)
                                                              (apply-k k^^ k^) ;;apply-k
                                                              (apply-env env-cps y k^^))) k^)] ;;apply-env
      ...
      [`(let ,e ,body) (value-of-cps e env-cps
                                     (lambda (a)
                                       (value-of-cps body (lambda (y k^^) (if (zero? y)
                                                                              (apply-k k^^ a) ;;apply-k
                                                                              (apply-env env-cps (sub1 y)))) k^)))] ;;apply-env
      [`(var ,expr) (apply-env env-cps expr k^)] ;;apply-env
      [`(lambda ,body) (apply-k k^ (lambda (a k^^) (value-of-cps body (lambda (y k^^^) (if (zero? y) ;;apply-k
                                                                                   (apply-k k^^^ a) ;;apply-k
                                                                                   (apply-env env-cps (sub1 y) k^^^))) k^^)))] ;;apply-env
