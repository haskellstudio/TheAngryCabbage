...
(define value-of-cps
  (lambda (expr env-cps k^)
    (match expr
      ...
      [`(lambda ,body) (apply-k k^(lambda (a k^^) (value-of-cps body (extend-env env-cps body a) k^^)))] ;;extend environment
...
(define apply-env
  (lambda (env-cps y k)
    ...
    [`(extend-env ,env-cps ,x ,arg) (if (zero? y) (apply-k k arg) (apply-env env-cps (sub1 y) k))])))
