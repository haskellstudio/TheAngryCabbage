...
(define value-of-cps
  (lambda (expr env-cps k^)
    (match expr
      ...
      [`(lambda ,body) (apply-k k^ (make-closure env-cps body))] ;;make-closure
...
;;unsure of whether to pass k^ through to this
(define make-closure
  (lambda (env-cps body)
    (lambda (arg k)
      (value-of-cps body (extend-env env-cps body arg) k))))
